function [min_dist,best_tour,generation] = mdmtspv_MUTATED_ga(xy,max_salesmen,depots,CostType,min_tour,pop_size,num_iter,show_prog,show_res,dmat)
...
for k = nargin:nargs-1
    switch k
        case 0
            xy = [-2    2; 0    2; 2    2; -1    0; 1    0];
            %xy = 10*rand(40,2); 
        case 1
            max_salesmen=2;
        case 2
            depots = [-1, 1; 1, 1];
%             depots = xy(1:max_salesmen, :);
%             if isempty(dep)
%                %depots = xy(1:max_salesmen, :);
%                depots = [-1, 1; 1, 1];
%             else
...
pop_size = max(16,16*ceil(pop_size(1)/16));
...
if show_prog
%     pfig = figure('Name','MTSPV_OURS_GA | Current Best Solution','Numbertitle','off');
end
%pause(10);
iter=0;
iter2go=0;
    ...
    for p = 1:pop_size
        ...
        for sa = 1:salesmen
            if rng(sa,1)<=rng(sa,2)
                Tour=[sa p_rte(rng(sa,1):rng(sa,2))];
                indices=length(Tour);
...
%                 title(sprintf(['Current Minimum Longest Distance = %1.4f, Salesmen = %d, ' ...
...
%             hold off
    ...
    for p = ops:ops:pop_size
        ...
        best_of_16_rte = rtes(idx,:);
        best_of_16_brk = brks{idx};
        rte_ins_pts = sort(ceil(n*rand(1,2)));
        I = rte_ins_pts(1);
        J = rte_ins_pts(2);
        for k = 1:ops % Generate New Solutions
            tmp_pop_rte(k,:) = best_of_16_rte;
            tmp_pop_brk{k} = best_of_16_brk;
            switch k
                case 2 % Flip
...
%                     tmp_pop_brk{k} = temp;
                case 12
                    l = random('unid', max_salesmen);
                    if (l == 1)
                        temp=tmp_pop_brk{k};
                        temp=[0 temp(2:end)];
                        tmp_pop_brk{k}=temp;
                    elseif (l == max_salesmen)
                        temp=tmp_pop_brk{k};
                        temp=[temp(1:end-1) n];
                        tmp_pop_brk{k}=temp;
                    else
...
%                     tmp_pop_brk{k}=temp;
                case 14 %swap close points
                    if I<n
                        tmp_pop_rte(k,[I I+1]) = tmp_pop_rte(k,[I+1 I]);
                    end
                case 15
                    if J > 1
                        tmp_pop_rte(k,[J J-1]) = tmp_pop_rte(k,[J-1 J]);
                    end
                otherwise
            end
        end
...
for i=1:max_salesmen
    if rng(i,1)<=rng(i,2)
        best_tour(i,1:(rng(i,2)-rng(i,1)+1))=[opt_rte([rng(i,1):rng(i,2)])];
        %dep(i, :) = xy(best_tour(i, end), :);
    else
        best_tour(i, 1)=0; %%%
        %dep(i, :) = xy(best_tour(i, end), :);
    end
...
for c=2:indices-1
    VehicleTourLength=VehicleTourLength+d(Tour(c),Tour(c+1));
end
%VehicleTourLength=VehicleTourLength+d0(Tour(indices+1),Tour(indices));
%% outputs a random vector of breaks that divides the route vector between agents
function breaks = randbreak(num_rob, numOfStores, min_tour)
% get number of breaks given the number of robots
numBrks = num_rob - 1;
% generate a random array (size: 1xnumBrks) from a uniform distribution with max value being the number of stores and min value being 0
breaks = sort(random('unid', numOfStores + 1, 1, numBrks) - 1);

%% for each robot, calculates the start and end indices specifying the range of stores assigned to it from the route vector
% if the start index is greater than the end index, it means that this robot is not assigned any stores
function rngVec = CalcRange(brkVector, numOfStores)

% the brkVector is empty
if isempty(brkVector)
    % assign all stores to the single robot we have and return
    rngVec(1,:) = [1 numOfStores];
    return;
end

% initialize a flag specifying whether we are processing the first element in the brkVector
isFirstElem = 1;

% for every element in the brkVector
for i = 1:length(brkVector)
    % if we are processing the first element of brkVector
    if isFirstElem == 1
        % set the start index for this first robot to 1 and the end index to the one specified by the current element of brkVector
        rngVec(i,:) = [1 brkVector(i)];
        % set the flag to 0 indicating that we are done with the first element
        isFirstElem = 0;
    % if we are processing intermediate elements of brkVector or the last one
    else
        % set the start index as the one following the previous brkVector element and the end index as the current brkVector element
        rngVec(i,:) = [brkVector(i-1) + 1 brkVector(i)];
    end        
end

% determine the start and end indices of the last robot
% if not all stores are yet assigned
if brkVector(end) < numOfStores
    % set the start index as the one following the last brkVector element and the end index as the number of stores
    rngVec(i+1,:)=[brkVector(end)+1 numOfStores];
% otherwise
else
    % let the start index for this robot be greater than the end index (meaning the last robot is not assigned any stores)
    rngVec(i+1,:) = [brkVector(end) numOfStores-1];
end